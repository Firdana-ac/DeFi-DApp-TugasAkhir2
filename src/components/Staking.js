...
const Staking = (props) => {
  ...
  return (
    <div className={classes.Staking}>
      ...
      <h1> DANZE Token Staking DApp</h1>
      <p>{props.account}</p>
      <h3>
        {props.apy}% (APY) - {props.apy / 365}% Penghasilan Harian
      </h3>
      ...
      <div className={classes.totals}>
        <h4>
          Total Staked (by all users): {props.totalStaked} DanzeToken (DNZ)
        </h4>
        <div>&nbsp;</div>
        <h5>My Stake: {props.myStake} DanzeToken (DNZ) </h5>
        <h5>
          My Estimated Reward:{' '}
          {((props.myStake * props.apy) / 36500).toFixed(3)} DanzeToken (DNZ)
        </h5>
        <h5 onClick={goMax} className={classes.goMax}>
          My balance: {props.userBalance} DanzeToken (DNZ)
        </h5>
      </div>
    </div>
  );
};
//My balance: 504304.394968082 DanzeToken (Tst)
